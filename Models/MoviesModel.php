...
class MoviesModel{
	...
	public function AddMovie($Titulo,$Fecha,$Productor,$Descricion,$Id_genero_fk,$imagen = null){
	 	$pathImg = null;
        if ($imagen)
            $pathImg = $this->uploadImage($imagen);

	 	$query = $this->db->prepare('INSERT INTO  pelicula (Titulo, Fecha, Productor, Descripcion, id_genero_fk,Img) VALUES (?,?,?,?,?,?)');
	 	$query->execute([$Titulo,$Fecha,$Productor,$Descricion,$Id_genero_fk,$pathImg]);
	}
	...
	public function UpdateMovie($Titulo,$Fecha,$Productor,$Descricion,$Id_genero_fk,$imagen,$id){
	 	$pathImg = null;
        if ($imagen)
            $pathImg = $this->uploadImage($imagen);

	 	$query = $this->db->prepare('UPDATE pelicula SET Titulo= ? , Fecha = ?, Productor= ? , Descripcion= ?, id_genero_fk=?,Img=? WHERE ID = ?');
	 	$query->execute([$Titulo,$Fecha,$Productor,$Descricion,$Id_genero_fk,$pathImg,$id]);
	}
